...
export default function EmployeePage() {
	...
	const edit_personal_info_mutation = useMutation({
		mutationFn: async (payload: Partial<TEmployee>) => {
			const response = await api.put(`/employees/${employee_id}?type=personal_info`, payload);
			return response.data;
		},
		onSuccess: () => {
	...
	const edit_contact_info_mutation = useMutation({
		mutationFn: async (payload: Partial<TEmployee>) => {
			const response = await api.put(`/employees/${employee_id}?type=contact_info`, payload);
			return response.data;
		},
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ['employee', employee_id] });
	...
	const edit_relatives_mutation = useMutation({
		mutationFn: async (payload: TEmployeeRelative[]) => {
			const response = await api.put(`/employees/${employee_id}?type=relatives`, { relatives: payload });
			return response.data;
		},
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ['employee', employee_id] });
	...
	const edit_salary_mutation = useMutation({
		mutationFn: async (amount: number) => {
			const response = await api.put(`/employees/${employee_id}?type=salary`, { salary: { amount: amount } });
			return response.data;
		},
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ['employee', employee_id] });
	...
	const add_pagibig_mutation = useMutation({
		mutationFn: async (payload: Partial<TPagIBIGSettings>) => {
			const response = await api.post(`/employees/${employee_id}/pagibig`, payload);
			return response.data;
		},
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ['employee', employee_id] });
	...
	const add_sss_mutation = useMutation({
		mutationFn: async (payload: Partial<TSSSSettings>) => {
			const response = await api.post(`/employees/${employee_id}/sss`, payload);
			return response.data;
		},
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ['employee', employee_id] });
	...
	const add_philhealth_mutation = useMutation({
		mutationFn: async (payload: Partial<TPhilhealthSettings>) => {
			const response = await api.post(`/employees/${employee_id}/philhealth`, payload);
			return response.data;
		},
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ['employee', employee_id] });
	...
	const add_bir_mutation = useMutation({
		mutationFn: async (payload: Partial<TBIRSettings>) => {
			const response = await api.post(`/employees/${employee_id}/bir`, payload);
			return response.data;
		},
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ['employee', employee_id] });
	...
	const edit_sss_mutation = useMutation({
		mutationFn: async (payload: Partial<TSSSSettings>) => {
			const response = await api.put(`/employees/${employee_id}?type=sss_settings`, payload);
			return response.data;
		},
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ['employee', employee_id] });
	...
	const edit_philhealth_mutation = useMutation({
		mutationFn: async (payload: Partial<TPhilhealthSettings>) => {
			const response = await api.put(`/employees/${employee_id}?type=philhealth_settings`, payload);
			return response.data;
		},
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ['employee', employee_id] });
	...
	const edit_pagibig_mutation = useMutation({
		mutationFn: async (payload: Partial<TPagIBIGSettings>) => {
			const response = await api.put(`/employees/${employee_id}?type=pagibig_settings`, payload);
			return response.data;
		},
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ['employee', employee_id] });
	...
	const edit_bir_mutation = useMutation({
		mutationFn: async (payload: Partial<TBIRSettings>) => {
			const response = await api.put(`/employees/${employee_id}?type=bir_settings`, payload);
			return response.data;
		},
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ['employee', employee_id] });
	...
	const archive_employee_mutation = useMutation({
		mutationFn: async (employee_id: string) => {
			const response = await api.put(`/employees/${employee_id}?type=archive`);
	...
	const handleArchiveEmployee = () => {
		archive_employee_mutation.mutate(employee_id);
	};
		...
		<>
			<main className="bg-slate-50 min-h-screen w-full pb-12">
				<div className="container mx-auto px-4 py-8">
					<div>
						<button onClick={() => router.push('./')} className="rounded-full bg-white p-2 border border-slate-200 hover:bg-slate-50">
										...
										<div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
											...
											<div>
												...
												<p className="text-sm font-medium text-slate-900 mt-1">
													PHP {FormatAsMoney(employee?.philhealth_settings.contribution?.total.toString() || '0')}
												</p>
											</div>
											<div>
												<p className="text-xs text-slate-600 uppercase font-semibold">No. of Contribution</p>
									...
									<div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
										...
										<div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
											...
											<div>
												<p className="text-xs text-slate-600 uppercase font-semibold">Contribution</p>
												<p className="text-sm font-medium text-slate-900 mt-1">
													PHP {FormatAsMoney(employee?.pagibig_settings.contribution?.total.toString() || '0')}
												</p>
											</div>
											<div>
												<p className="text-xs text-slate-600 uppercase font-semibold">No. of Contribution</p>
										...
										<div key={`${log.title}-${index}`} className="border border-slate-200 rounded-lg p-4 bg-slate-50">
											<div className="flex items-start justify-between gap-4">
												...
												<div className="text-right">
													...
													<p className="text-sm font-semibold text-slate-900">PHP {FormatAsMoney(log.net_pay.toString())}</p>
												</div>
											</div>
											<div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs text-slate-600">
												<span>Gross: PHP {FormatAsMoney(log.gross_pay.toString())}</span>
												<span>
						...
						<div className="flex items-center justify-between gap-2 p-4">
							<div className="max-w-md">
								...
								<p className="text-sm text-slate-500">
									Delete the record permanently or archive it for future reference. Archived records can be restored later if needed.
								</p>
							</div>
							<Button
								text="Delete"
										...
										{payrollLogsSlice.map((log, index) => (
											<div key={`${log.title}-${index}`} className="border border-slate-200 rounded-lg p-4">
												<div className="flex items-start justify-between gap-4">
													...
													<div className="text-right">
														<p className="text-xs text-slate-500">Net Pay</p>
														<p className="text-sm font-semibold text-slate-900">PHP {FormatAsMoney(log.net_pay.toString() || '0')}</p>
													</div>
												</div>
												<div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs text-slate-600">
													<span>Gross: PHP {FormatAsMoney(log.gross_pay.toString() || '0')}</span>
													<span>
														Month: {log.payroll_month} {log.payroll_year}
													</span>
													<span>
														Benefits:{' '}
														{log.benefits.length ? log.benefits.map((benefit) => benefit.benefit_key.toUpperCase()).join(', ') : 'None'}
													</span>
												</div>
											</div>
										))}
								...
								<div className="space-y-6">
									...
									<div>
										...
										{salaryFormData.length === 0 ? (
											...
											<div className="space-y-3">
												{salaryFormData.slice().map((salary, reverseIndex) => {
