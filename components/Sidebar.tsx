								...
								<div className="space-y-2">
									{filtered_links.map((link) => (
										<div key={link.id}>
											<Link
												href={link.href}
												className={`w-full flex items-center px-4 py-2 text-slate-500 rounded-md hover:bg-gray-50 ${pathname === link.href ? 'bg-gray-50 text-slate-700' : ''}`}
											>
												{link.icon}
									...
									<div>
										<button
											...
											className={`w-full flex items-center px-4 py-2 text-slate-500 rounded-md hover:bg-gray-50 ${pathname === '/dashboard/settings' ? 'bg-gray-50 text-slate-700' : ''}`}
										>
											<LogOut size={16} />
					...
					<>
						<div className="flex flex-col w-20 px-4 py-2 overflow-y-auto bg-white border-r border-r-slate-100">
							<div className="flex flex-col justify-start space-y-3 flex-1 mt-6">
								{filtered_links.map((link) => (
									<Link href={link.href} key={link.id} className="w-full flex items-center justify-center px-2 py-2 text-slate-500 rounded-md hover:bg-gray-100">
										{link.icon}
									</Link>
								))}
							</div>
